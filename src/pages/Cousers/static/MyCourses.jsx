  ...
  return (
    <div className='container' id='cursos'>
      ...
      <div className='carousel' ref={carousel}>
        {data.map(item => {
          const { id, name_c, desc_c, tempo_c, image_path } = item; // Adicione image_path aqui
          return (
            <div className='item' key={id} onClick={() => openModal(item)}>
              <div className='image'>
                <img src={`http://localhost/backend/public/uploads/${image_path}`} alt="Course" /> {/* Substitua pelo caminho correto para sua imagem */}
              </div>
              <div className='info'>
                <h3>{name_c}</h3>
                <p>{desc_c}</p>
